...
class OrderbookSync extends WebsocketClient {
  ...
  _newProduct(productID) {
    ...
    this._circular_buffers[productID] = new CircularBuffer(15000);
  ...
  processMessage(data,index) {
    ...
    if (this._sequences[product_id] === -2) {
      ...
      this._circular_buffers[product_id] = new CircularBuffer(15000);
    ...
    const circular_buffer = this._circular_buffers[product_id];
